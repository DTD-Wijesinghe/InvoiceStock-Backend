...
def main():
    email = (
        input("Administrator email [daniduwijesingheofficial@gmail.com]: ")
        .strip()
        .lower()
        or "daniduwijesingheofficial@gmail.com"
    )

    name = (
        input("Administrator name [D.T.D.Wijesinghe]: ").strip()
        or "D.T.D.Wijesinghe"
    )

    if "@" not in email or len(name) < 2:
        raise SystemExit("Valid email and name required.")
    ...
    with SessionLocal.begin() as db:
        if db.scalar(select(User).where(User.email == email)):
            raise SystemExit("Email already exists. No account changed.")
